...
int check_gate(node ** nodes); // 1 is deadlock, 0 is false positive
void free_gate(pthread_mutex_t ** gate_t);
pthread_mutex_t ** get_gates(node ** nodes, int index);
int in_gate(pthread_mutex_t ** gate_t, pthread_mutex_t * mutex);
...
/*
	char * first = "LD_PRELOAD=\"./pred_ddmon.so\" ./";
...
void init_node(node* n) {
	...
	n->gate = (pthread_mutex_t ***) malloc(sizeof(pthread_mutex_t**)*NN);
	for(int i=0; i<NN; i++){
		n->edges[i] = 0x0;
		n->gate[i] = 0x0;
		n->lines[i] = 0;
...
void dfs_visit(node ** nodes, node * n, int * cycle){
	...
	for(int i=0; i< NN; i++){
		...
		if(n->edges[i]->col == Gray){
			//printf("cycle: %p\n", n);
			if(check_cycle(nodes) == 1 && check_gate(nodes) == 1){	
				*cycle = 1;
				return;
			}
		}
		...
		if(nodes[i]->thread_id == thread_id && i != found && nodes[i]->line != 0){ // with the same thread id
			...
			for(int j=0; j<NN; j++){
				if(nodes[i]->edges[j] == nodes[found]){ // already has an invalid edge
					...
					free_gate(nodes[i]->gate[j]);
					nodes[i]->gate[j] = get_gates(nodes, i);
					have = 1;
					break;
				}
			}
...
int check_cycle(node ** nodes){
	...
	// for the edges from the Gray node to the Gray node
...
int check_gate(node ** nodes){
	pthread_mutex_t ** gate_t = 0x0;
	node* first_node = 0x0;
	node* next = 0x0;
	...
	for(int i=0;i<NN; i++){
		if(nodes[i] != 0x0 && nodes[i]->col == Gray){
			...
			for(int j=0; j<NN; j++){
				if(nodes[i]->edges[j] != 0x0 && nodes[i]->edges[j]->col == Gray){
					// save the gate of the edge and the next node
					gate_t = nodes[i]->gate[j];
					next = nodes[i]->edges[j];
				//	printf("first node that is gray: nodes[%d]\n", i); 
				//	printf("next node that is gray: %p\n", nodes[i]->edges[j]); 
					break;
				}
			}
		}
	...
	while(first_node != next){
		for(int i=0; i<NN; i++){ // for all edges in the next node
			if(next->edges[i] != 0x0 && next->edges[i]->col == Gray){ // find the one that points to Gray node
				// check whether the gate of the edge has same mutexes

				// for all mutexes in gate_t
				for(int j=0; j<NN; j++){
					// if the mutex in gate_t is not in the gate of the edge
					if(gate_t[j] != 0x0 && in_gate(next->gate[i], gate_t[j]) == 0){ 
						// delete that mutes from gate_t
						gate_t[j] = 0x0;
					}
				}
				next = next->edges[i];
...
// returns the array of mutexes that should point out of the node
pthread_mutex_t ** get_gates(node ** nodes, int index){
	//printf("get_gates for nodes[%d]\n", index);
		
	pthread_mutex_t ** gate_t = (pthread_mutex_t **) malloc(sizeof(pthread_mutex_t*)*NN);
	for(int i=0; i<NN; i++){
		gate_t[i] = 0x0;
	}
	int ind=0;
	while(gate_t[ind] != 0x0){
		ind++;
	}

	for(int i=0; i<NN; i++){ // for all mutex nodes,
		if(nodes[i] == 0x0 || i == index || nodes[i]->thread_id != nodes[index]->thread_id) continue; // no node or the current node or anther thread node, continue
		for(int j=0; j<NN; j++){ // for all edges
			// if there an valid edge, pointing to the node from the node w/ the same thread
			if(nodes[i]->edges[j] != 0x0 && nodes[i]->valid[j] == 1 && nodes[i]->edges[j] == nodes[index]){
				// add the mutexes of the edge to the new gate_t
				//printf("nodes[%d]->edges[%d] points to node[%d]\n", i, j, index);
				for(int k=0; k<NN; k++){
					// if there is a gate & there is a mutex in gate
					if(nodes[i]->gate[j] != 0x0 && nodes[i]->gate[j][k] != 0x0){
						// check whether the mutex is in gate_t
						if(in_gate(gate_t, nodes[i]->gate[j][k]) == 0){
							//printf("new mutex\n");
							gate_t[ind++] = nodes[i]->gate[j][k];
						}
					}
				}
				break;
	...
	gate_t[ind] = nodes[index]->mutex;
	
	//printf("gate_t -> nodes[%d]: %p\n", index, nodes[index]);
	//for(int i=0; i<NN; i++){
	//	printf("gate_t[%d] = %p\n", i, gate_t[i]);
	//}
	return gate_t;
...
// 1: the mutex exists in the gate_t
// 0: the mutes is not in the gate_t
int in_gate(pthread_mutex_t ** gate_t, pthread_mutex_t * mutex){
	for(int i=0; i<NN; i++){
		if(gate_t[i] == mutex){
			return 1;
